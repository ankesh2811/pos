...
?>
    ...
    <div class="content-wrap">
                ...
                <div class="row">
                    <div class="col-lg-4 p-0">
                        <div class="page-header">
							...
							<?php 
							$l1 = mysqli_query($connection,"SELECT SUM(amount) as sale FROM orders");
							$l1_data = mysqli_fetch_array($l1);
							$sale = $l1_data['sale'];
							
							$l2 = mysqli_query($connection,"SELECT count(id) as t FROM orders");
							$l2_data = mysqli_fetch_array($l2);
							$t = $l2_data['t'];
        ...
        <div class="col-lg-6 col-md-6 col-sm-6 col-xs-6" style="background-color:#d2d2d2;"><div class="stat-text">Total Orders</div>
												<div class="stat-digit"><?php $a1 = mysqli_query($connection,"SELECT COUNT(id) as c_id FROM orders");  $a1_data = mysqli_fetch_array($a1);   $c_id = $a1_data['c_id']; echo $c_id; ?></div></div>
        <div class="col-lg-6 col-md-6 col-sm-6 col-xs-6" style="background-color:#d2d2d2;"><div class="stat-text">Net Sales</div>
												<div class="stat-digit"><?php
												
												$a11 = mysqli_query($connection,"SELECT SUM(discount_price) as d FROM orders"); 
												$a11_data = mysqli_fetch_array($a11);   
												$d = $a11_data['d'];
												$a12 = mysqli_query($connection,"SELECT SUM(taxes) as tax FROM orders"); 
												$a12_data = mysqli_fetch_array($a12);   
												$tax = $a12_data['tax'];
												$dk12 = mysqli_query($connection,"SELECT SUM(delivery) as del FROM orders");
										$dk12_data = mysqli_fetch_array($dk12);
										
												$k = $d+$tax+$dk12_data['del'];


												if(!empty($k)) { echo round($k); } else { echo '0'; } ?></div></div>
      </div>
      <div class="row" style="margin-top:2%;margin-bottom:3%;">
